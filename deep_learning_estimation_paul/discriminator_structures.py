...
from keras.layers import (Layer, Conv2D, Conv2DTranspose, LeakyReLU, Activation, Concatenate, Dropout, \
    ...
    LayerNormalization, GroupNormalization, SpectralNormalization)
from deep_learning_based_estimation_of_heart_surface_potentials_main.show_data import *
from tensorflow.keras.regularizers import l2

# define the discriminator model
def define_1x1_patch_spectralnorm_discriminator(image_shape, n_filters=32, activation='relu', initial_lr=0.001, decay_rate=0.96):
    # weights init
    ...
    d = Conv2D(n_filters, (3, 3), strides=(2, 2), padding='same', kernel_initializer=init, kernel_regularizer=l2(1e-4))(merged)
    d = LeakyReLU(negative_slope=0.2)(d)

    d = SpectralNormalization( Conv2D(n_filters * 2, (3, 3), strides=(2, 2), padding='same', kernel_initializer=init, kernel_regularizer=l2(1e-4))(d))
    d = LeakyReLU(negative_slope=0.2)(d)
    ...
    # patch output
    d = Conv2D(1, (2, 2), padding='valid', kernel_initializer=init, kernel_regularizer=l2(1e-4))(d)
    ...
    model.compile(loss=tf.keras.losses.BinaryCrossentropy(label_smoothing=0.2), optimizer=opt, loss_weights=[0.5])
    return model


# define the discriminator model
def define_1x1_patch_l2norm_discriminator(image_shape, n_filters=32, activation='relu', initial_lr=0.001, decay_rate=0.96):
    # weights init
    init = RandomNormal(stddev=0.02)
    ...
    d = Conv2D(n_filters * 2, (3, 3), strides=(2, 2), padding='same', kernel_initializer=init, kernel_regularizer=l2(1e-4))(d)
    d = BatchNormalization()(d)
    d = LeakyReLU(negative_slope=0.2)(d)

    d = Conv2D(n_filters * 4, (3, 3), strides=(2, 2), padding='same', kernel_initializer=init, kernel_regularizer=l2(1e-4))(d)
    d = BatchNormalization()(d)
    d = LeakyReLU(negative_slope=0.2)(d)

    d = Conv2D(n_filters * 8, (3, 3), strides=(2, 2), padding='same', kernel_initializer=init, kernel_regularizer=l2(1e-4))(d)
    d = BatchNormalization()(d)
    d = LeakyReLU(negative_slope=0.2)(d)

    d = Conv2D(n_filters * 16, (3, 3), strides=(2, 2), padding='same', kernel_initializer=init, kernel_regularizer=l2(1e-4))(d)
    d = BatchNormalization()(d)
    d = LeakyReLU(negative_slope=0.2)(d)
...
# define the discriminator model
def define_1x1_patch_labelsmoothing_discriminator(image_shape, n_filters=32, activation='relu', initial_lr=0.001, decay_rate = 0.96):
    # weights init
    init = RandomNormal(stddev=0.02)
